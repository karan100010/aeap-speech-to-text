		...
		setTimeout(() => {
			...
			axios.post(url, req)
				.then(response => {
					...
					let trasurl="http://192.168.56.108:5007/translate"
				...
				.then(response => {
					console.log(response.data);
					this.nlp.push(response.data);
				})
					let result = {
						text: response.data.transcribe+" "+this.nlp[0].entities[0].entity+" "+this.nlp[0].entities[0].value,
