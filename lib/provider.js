...
class GoogleProvider extends Writable {
	...
	_write(chunk, encoding, callback) { 
		...
		// check if the total duration of the stream is 7 seconds
		const totalDuration = this.chunks.reduce((duration, chunk) => duration + chunk.duration, 0);
		if (totalDuration >= 7) {
			// combine all buffers in the chunks array into a single buffer
			const streamBuffer = Buffer.concat(this.chunks);
			
			// send the stream buffer to the provider at port 5001
			const url = '192.168.56.1:5000/convert';
			axios.post(url, streamBuffer)
				.then(response => {
...
_writev(){
	//set chunks for 7 scoends from when the first chunk is received
	...
	setTimeout(() => {
		...
		//send chuks to provider
		const url2 = 'http://192.168.56.1:5005/convert';
		axios.post(url, req)
			.then(response => {	
				console.log(response.data);
				callback();
